...
```{r}
# Load the faraway package for the Ozone dataset
library(faraway)

# it is loaded in the "package environment"
# to "see" it in the Global environment
data("ozone")
```

The variable names are as follows:

| Variable Name | Description             | Model Part
|---------------|-------------------------|-----------------------|
| O3            | Daily Ozone Level       | target - outcome      |
| vh            | Pressure height         | predictor - covariate |
| wind          | Wind speed at LAX       | predictor - covariate |
| temp          | Temperature             | predictor - covariate |
| ibh           | Inversion base height   | predictor - covariate |
| dpg           | Pressure gradient       | predictor - covariate |
| ibt           | Inversion base temp     | predictor - covariate |
| vis           | Visibility at LAX       | predictor - covariate |
| doy           | Day of year             | predictor - covariate |
...
# this uses melt from reshape to restructure the data
ozonem <- melt(ozone, id.vars="O3")

# now we can plot all "values" for the 
# list of predictors/covariates
...
We can also get the correlations.

```{r}
# you do not need to restructure
library(correlation)
cor1 <- correlation(ozone)

# lets just view the correlations with O3
# sort the absolute values of correlation
# in descending order to see the highest at top
library(dplyr)
cor1 %>%
  filter(Parameter1 == "O3") %>%
  arrange(desc(abs(r)))
```
...
```{r}
# Overall variability of ozone - get the variance
var(ozone$O3)
```

Since temp is listed as one of the most "important" (highest correlation) with ozone, what if we split the sample at the median temp of 62 (see summary stats above) - now look at the variance of ozone again.
...
Notice the "tree" starts with a split for temp < 67.5 which is a little above the median. But the O3 value shown at the split is 11.78 which is the mean O3 (see summary stats above).

Then the tree splits again at:

* temp < 58.5 for the lower branch
* temp < 79.5 for the upper branch

So we end up here with 4 "leaves":

1. n=138, avg O3 5.877 for temp from 25(min)-58.5
2. n=76, avg O3 10.24 for temp from 58.5-67.5
3. n=72, avg O3 17.17 for temp from 67.5-79.5
4. n=44, avg O3 24.11 for temp from 79.5-93(max)

```{r}
plot(fittemp, uniform = TRUE, 
     compress = FALSE, margin = 0.05)
text(fittemp, use.n = TRUE, all = TRUE, cex = 0.8)
```
...
plot(fitall, uniform = TRUE, 
     compress = FALSE, 
     margin = 0.05,
     main = "Regression Tree for Ozone Dataset")
...
# Prune the tree
pfit <- 
  prune(fitall, cp = fitall$cptable[which.min(fitall$cptable[, "xerror"]), "CP"])

# Plot the pruned tree
plot(pfit, uniform = TRUE, compress = FALSE, 
     margin = 0.05,
     main = "Pruned Regression Tree for Ozone")
text(pfit, use.n = TRUE, 
     all = TRUE, cex = 0.8)
```

The package *party* provides nonparametric regression trees, and it also creates better graphics. Let's give it a try.
...
**NOTE**: The `kyphosis` dataset is in the `rpart` package.
...
# MUST ADD family = binomial
# otherwise this runs a linear regression
glm(kyph ~ Age + Number + Start, 
    family = binomial,
    data = mykyph)
```
...
class(fitallpk)
plot(fitallpk, main = "Conditional Inference Tree for Kyphosis")
```
